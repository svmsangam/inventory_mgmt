        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    <a href="${pageContext.request.contextPath}/order/list" style="float:left; margin-bottom:8px;margin-right: 150px;"><button class="btn btn-primary btn-sm">List Order</button></a>
                    <c:if test="${order.saleTrack eq 'DELIVERED'}">
                        <button class="btn btn-primary pull-right" id="print"><i class="fa fa-print"></i> Print Invoice</button>
                    </c:if>
                        ...
                        <c:choose>
                            <c:when test="${order.orderType eq 'Sale'}">
                                <tr>
                                    ...
                                    <c:if test="${order.saleTrack eq 'NULL'}">
                                        <td><input type="radio" class="track" name="saleTrack" value="NULL" mycheck="yes" checked="checked" /></td>
                                        <td><input type="radio" class="track" name="saleTrack" url="${pageContext.request.contextPath}/order/saletrack" value="ACCEPTED" /></td>
                                        <td><input type="radio" class="track" name="saleTrack" url="${pageContext.request.contextPath}/order/saletrack" value="CANCEL" /></td>
                                    </c:if>


                                    <c:if test="${order.saleTrack eq 'ACCEPTED'}">
                                        <td><input type="radio" class="track" name="saleTrack" value="ACCEPTED" mycheck="yes" checked="checked" /></td>
                                        <td><input type="radio" class="track" url="${pageContext.request.contextPath}/order/saletrack" name="saleTrack" value="PACKED" /></td>
                                    </c:if>

                                    <c:if test="${order.saleTrack eq 'PACKED'}">
                                        <td><input type="radio" class="track" name="saleTrack" value="PACKED" mycheck="yes" checked="checked" /></td>
                                        <td><input type="radio" class="track" url="${pageContext.request.contextPath}/order/saletrack" name="saleTrack" value="SHIPPED" /></td>
                                    </c:if>

                                    <c:if test="${order.saleTrack eq 'SHIPPED'}">
                                        <td><input type="radio" class="track" name="saleTrack" value="SHIPPED" mycheck="yes" checked="checked"/></td>
                                        <td><input type="radio" class="track" url="${pageContext.request.contextPath}/order/saletrack" name="saleTrack" value="DELIVERED" /></td>
                                    </c:if>
                            ...
                            <c:otherwise>
                                    ...
                                    <td>
                                        <c:if test="${order.purchaseTrack eq 'ISSUED'}">
                                            <input type="radio" class="track" name="purchaseTrack" value="ISSUED" mycheck="yes" checked="checked" />
                                        </c:if>
                                        <c:if test="${order.purchaseTrack ne 'ISSUED'}">
                                            <input type="radio" url="${pageContext.request.contextPath}/order/purchasetrack" class="track" name="purchaseTrack" value="ISSUED" />
                                        </c:if>
                                    </td>
                                    <td>
                                        <c:if test="${order.purchaseTrack eq 'RECEIVED'}">
                                            <input type="radio" class="track" name="purchaseTrack" mycheck="yes" value="RECEIVED" checked="checked" />
                                        </c:if>
                                        <c:if test="${order.purchaseTrack ne 'RECEIVED'}">
                                            <input type="radio" class="track" url="${pageContext.request.contextPath}/order/purchasetrack" name="purchaseTrack" value="RECEIVED" />
                                        </c:if>
                                    </td>
                                ...
                                <td>
                                    ...
                                    <c:if test="${order.purchaseTrack eq 'CANCELLED'}">
                                        <input type="radio" class="track" name="purchaseTrack" mycheck="yes" value="CANCELLED" checked="checked" />
                                    </c:if>
                                    <c:if test="${order.purchaseTrack ne 'CANCELLED'}">
                                        <input type="radio" class="track" url="${pageContext.request.contextPath}/order/purchasetrack" name="purchaseTrack" value="CANCELLED" />
                                    </c:if>
                                </td>
                                    ...
                                    <table class="table table-hover">
                                        ...
                                        <c:forEach var="orderItem" items="${orderItemList}">
                                            <tr>
                                                ...
                                                <td><fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${orderItem.amount}" /></td>
                                            </tr>
                                        </c:forEach>
                                        </tbody>
                                        <tfoot>
                                        <tr>
                                            ...
                                            <th></th>
                                            <th></th>
                                            <th><fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${order.totalAmount}" /></th>
                                        </tr>
                                        <tr>
                                            <th>Tax(%)</th>
                                            <th></th>
                                        ...
                                        <tr>
                                            ...
                                            <th></th>
                                            <th></th>
                                            <th><fmt:formatNumber type = "number" maxFractionDigits = "3" value = "${order.grandTotal}" /></th>
                                        </tr>
                                        </tfoot>
                                    </table>
                                </c:if>
