...
<div class="panel-heading">
	<div class="col-md-offset-5">Edit Purchase Order
		<div class="pull-right">Purchase Order No. #${purchaseOrder.orderNo}</div>
		</div>
</div>
<div class="panel-body">
<form method="post" action="${pageContext.request.contextPath}/app/editpurchaseorder" modelAttribute="purchaseOrder" id="registerform" novalidate="novalidate">
...
<div class="row">
	...
	<div class="col-xs-4">
	<label name="purchaseOrderStatus">Status:</label>
			<select name="purchaseOrderStatus" class="select2">
				<c:forEach var="status" oldItems="${statusList}">
					<option value="${status}" <c:if test="${status == purchaseOrder.purchaseOrderStatus}">selected</c:if>>${status}</option>
				</c:forEach>
			</select>
	</div>
	<div class="col-xs-4 text-right">
		...
		<table class="table table-responsive table-striped table-bordered"
			...
			<c:if test="${fn:length(purchaseOrder.oldItemDtoList) gt 0}">
				...
				<tr>
				<td><span>${checkItem.itemName}</span><input type="hidden" name="oldItemList" value="${checkItem.id}"  required readOnly /></td>
   				<td><input type="number" id="input" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="form-control qty" onKeyup="update_amounts();"  value="${purchaseOrder.quantities[loop.index]}" name="quantity" placeholder="enter quantity" required/></td>
   				<td><input type="text" step="any" id="rate" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="form-control rate" value="${checkItem.rate}" name="rate" required readOnly/></td>
   				<td><input type="text" step="any" id="tax" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="form-control tax" value="${checkItem.tax}" name="tax"  required readOnly/></td>
		...
		<label class="lable">Total Cost </label> <input type="text" value="0.0"
			id="total" step="any"
	...
	<div>
		...
		<textarea name="notes" rows="4" class="form-control">${purchaseOrder.notes}</textarea>
	</div>
...
<div class="col-lg-12">
	<input type="submit" class="btn btn-primary pull-right" value="Update Purchase Order"/>
</div>
</form>
</div>
</div>
