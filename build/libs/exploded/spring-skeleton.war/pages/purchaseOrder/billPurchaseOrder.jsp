...
<form method="post" action="${pageContext.request.contextPath}/app/addbillpurchaseorder" modelAttribute="billDto" enctype="multipart/form-data" id="registerform" novalidate="novalidate">
	<input type="hidden" name="billNo" value="${billPurchaseOrder.billNo}"/>
	<div class="row">
	<div class="col-lg-3">
		<label name="purchaseOrderDto">Order No:</label>
		<select name="purchaseOrderDto.orderId" onChange="getPurchaseOrderValue();" id="load-values" class="form-control selectMe" required>	
		<option value="0" selected="selected">Select Order No</option>
		<c:forEach var="purchaseOrder" oldItems="${purchaseOrderList}">
			<option value="${purchaseOrder.orderId}" <c:if test="${billPurchaseOrder.purchaseOrderDto.orderId == purchaseOrder.orderId}">selected</c:if>>${purchaseOrder.orderNo}</option>
		</c:forEach>
		</select>
	</div>
	...
	<div class="col-lg-3">
		...
		<input class="form-control" placeholder="Vendor name" type="text" id="vendor" name="vendorDto.contactName" value="${billPurchaseOrder.vendorDto.contactName}" required />
		<p class="error">${error.contactName}</p>
	</div>
	
	<div class="col-lg-3">
		<label name="billDate">Bill Date:</label>
		<input type="text" placeholder="select date" id="billDate" class="datepicker form-control" name="billDate" value="${billPurchaseOrder.billDate}" required/>
		<p class="error">${error.billDate}</p>
	</div>
	
	<div class="col-lg-3">
		<label name="dueDate">Due Date:</label>
		<input type="text" placeholder="select date" id="dueDate"  class="datepicker form-control" name="dueDate" value="${billPurchaseOrder.dueDate}" required/>
	...
	<a id="add_row" class="btn btn-xs btn-primary pull-left"><span class="glyphicon glyphicon-plus"></span> Add Item</a>
		<table class="table table-responsive table-striped table-bordered"
			...
			<c:if test="${fn:length(billPurchaseOrder.oldItemDtoList) gt 0}">
				...
				<c:forEach var="checkItem" oldItems="${billPurchaseOrder.oldItemDtoList}" varStatus="loop">
				<tr>
				<td>
					<select id="js-example-responsive${loop.index}" class="form-control  mytest" arrt="${loop.index}" style="width: 100%" name="oldItemList" required >
   				...
   				<td><input type="number" id="input" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="form-control qty" onKeyup="update_amounts();"  value="${billPurchaseOrder.quantities[loop.index]}" name="quantity" placeholder="enter quantity" required/></td>
			...
			<c:forEach oldItems="${billPurchaseOrder.purchaseOrderDto.oldItemDtoList}" var="purchaseOrderItems">
				<tr>
					<td>${purchaseOrderItems.itemName}</td>
					<td>${purchaseOrderItems.quantity}<input type="hidden" class="form-control qty" value="${purchaseOrderItems.quantity}"/></td>
					<td>${purchaseOrderItems.rate}<input type="hidden" class="form-control rate" value="${purchaseOrderItems.rate}"/></td>
					<td>${purchaseOrderItems.tax}<input type="hidden" class="form-control tax" value="${purchaseOrderItems.tax}"/></td>
	...
	<div class="col-md-4">
		<label name="file">Attach Bill Image</label> 
		<input name="file" style="" type="file" class="form-control"/>
		<p class="error"></p>
	</div>

	<div class="form-group pull-right">
		<label class="lable">Total Cost </label> <input type="text" value="${billPurchaseOrder.totalCost}"	step="any"
			onkeypress="return (event.charCode >= 48 && event.charCode <= 57) || event.charCode == 46 || event.charCode == 0 "
			id="totalCost" class="form-control input-md" name="totalCost" readOnly
			required />
	</div>
	
	<div class="col-md-4">
		<label name="notes">Notes:</label><br/>
		<textarea name="notes" id="notes" rows="4" class="form-control">${billPurchaseOrder.notes}</textarea>
	</div>
...
$(document).ready(function() {
	...
	notes: {
        ...
        messages: {
        	...
        	notes: {
        		required: "Please provide a note for reference",
        		 maxlength: "note is small"
