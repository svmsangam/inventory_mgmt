...
import com.inventory.core.model.specification.ProductSpecification;
...
public class ProductInfoApi implements IProductInfoApi {
	...
	@Override
	public List<CategoryDomain> getAllCategory(long productInfoCategoryId, long storeId, Status status,
	...
	public List<ProductInfoDTO> list(Status status, long storeId) {
		return productInfoConverter
				.convertToDtoList(productInfoRepository.findAllByStatusAndStoreInfo(status, storeId));
	}
	...
	public List<ProductInfoDTO> list(Status status, long storeId, int page, int size) {
		...
		return productInfoConverter
				.convertToDtoList(productInfoRepository.findAllByStatusAndStoreInfo(status, storeId, pageable));
	}
	...
	@Override
	public List<ProductInfoDTO> filter(ProductFilterDTO filterDTO) {
		ProductSpecification specification = new ProductSpecification(filterDTO);
		Pageable pageable = createPageRequest(filterDTO.getPage(), filterDTO.getSize(), "id", Sort.Direction.DESC);
		return productInfoConverter.convertPageToDtoList(productInfoRepository.findAll(specification, pageable));
	}

	@Override
	public long filterCount(ProductFilterDTO filterDTO) {
		ProductSpecification specification = new ProductSpecification(filterDTO);
