...
<form method="post" action="${pageContext.request.contextPath}/app/editbillpurchaseorder" modelAttribute="billDto" enctype="multipart/form-data" id="registerform" novalidate="novalidate">
	...
	<div class="col-lg-4">
		...
		<select name="purchaseOrderDto.orderId" onChange="getPurchaseOrderValue();" id="load-values" class="form-control select2" required>	
		...
		<option value="0" <c:if test="${billPurchaseOrder.purchaseOrderDto.orderId eq 0}">selected="selected"</c:if>>Select Order No</option>
		</select>
	</div>
	
	<div class="col-lg-4">
		<label name="vendorDto" >Vendor: </label>
		<input type="text" id="vendor" class="form-control " name="vendorDto.contactName" value="${billPurchaseOrder.vendorDto.contactName}" required />
		<p class="error">${error.contactName}</p>
	</div>
	
	<div class="col-lg-4">
		<label name="billDate">Bill Date:</label>
		<input type="text" id="billDate" class="form-control datepicker"  name="billDate" value="${billPurchaseOrder.billDate}" required/>
		<p>${error.billDate}</p>
	</div>
	
	<div class="col-lg-4">
		<label name="dueDate">Due Date:</label>
		<input type="text" id="dueDate" class="form-control datepicker" name="dueDate" value="${billPurchaseOrder.dueDate}" required/>
		<p>${error.dueDate}</p>
	</div>
	</div>
	...
	<div id="inputDiv">
		<table class="table table-responsive table-striped table-bordered"
			...
			<c:forEach oldItems="${billPurchaseOrder.purchaseOrderDto.oldItemDtoList}" var="purchaseOrderItems" varStatus="loop">
				<tr>
					<td>${purchaseOrderItems.itemName}</td>
					<td>${purchaseOrderItems.quantity}<input type="hidden" class="qty" value="${billPurchaseOrder.purchaseOrderDto.quantities[loop.index]}"/></td>
					<td>${purchaseOrderItems.rate}<input type="hidden" class="rate" value="${purchaseOrderItems.rate}"/></td>
					<td>${purchaseOrderItems.tax}<input type="hidden" class="tax" value="${purchaseOrderItems.tax}"/></td>
