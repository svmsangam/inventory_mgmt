...
<c:if test="${not empty message}">
			...
			<form action="${pageContext.request.contextPath}/app/addinvoice"
				...
				<div class="input-group" id="inputDiv">
					<label class="lable">Invoice No * </label><input type="text"
						id="input" class="form-control input-md" value="${invoiceInfo.invoiceNo}" required readOnly/>
					<br />
				...
				<div class="input-group" id="inputDiv">
					<label class="lable">Date * </label> <input type="text"
						id="datepicker" name="invoiceDate" value="${invoiceInfo.invoiceDate }"
						required /> <br />
				...
				<div class="input-group" id="inputDiv">
					<label class="lable">Due Date * </label> <input type="text"
						id="datepicker2" name="dueDate" value="${invoiceInfo.dueDate }"
						required /> <br />
				...
				<script type="text/javascript">
						...
						$(document).ready(function(){
						    $("#add_row").click(function(){
						    	...
						    	var row = "<tr><td><select id='js-example-responsive' class='form-control input-md selectMe  mytest myclass"+count+"' arrt='"+count+"' style='width: 100%' name='oldItemIdList' ><option value='null' selected>select OldItem</option><c:forEach var='oldItem' oldItems='${oldItemList}'><option value='${oldItem.id}'>${oldItem.itemName}</option></c:forEach></select></td>";row += '<td><input type="number" id="quantity'+count+'" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="qty" onKeyup="update_amounts();"   name="quantity" placeholder="enter quantity" required/></td>';
					...
					<table class="table table-responsive table-striped table-bordered"
						...
						<c:if test="${fn:length(invoiceInfo.salesDto) gt 0}">
							...
							<c:forEach var="oldItem" oldItems="${invoiceInfo.salesDto.oldItemDtoList}" varStatus="loop">
								<tr>
								<td><span>${oldItem.itemName}</span><input type="hidden" name="oldItemList" value="${oldItem.id}"  required readOnly/></td>
								<td><input type="number" id="input" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="qty" onKeyup="update_amounts();"  value="${oldItem.quantity}" name="quantity" readOnly required/></td>
								<td><input type="number" step="any" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="rate" value="${oldItem.rate}" name="rate" required readOnly/></td>
		    				<td><input type="number" step="any" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="tax" value="${oldItem.tax}" name="tax"  required readOnly/></td>
		    				<td><span id="amount" class="amount">0</span></td><td> <a href="javascript:void(0);" class="remCF">Remove</a></td>
								</tr>
						</c:forEach>
						</c:if>
							...
							<c:forEach var="oldItem" oldItems="${invoiceInfo.oldItemList}" varStatus="loop">
								<tr>
								<td><span>${oldItem.itemName}</span><input type="hidden" name="oldItemList" value="${oldItem.id}"  required readOnly/></td>
								<td><input type="number" id="input" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="qty" onKeyup="update_amounts();"  value="${oldItem.quantity}" name="quantity" placeholder="enter quantity" required/></td>
								<td><input type="number" step="any" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="rate" value="${oldItem.rate}" name="rate" required readOnly/></td>
		    				<td><input type="number" step="any" onkeypress="return event.charCode > 47 && event.charCode < 58;" pattern="[0-9]{5}" class="tax" value="${oldItem.tax}" name="tax"  required readOnly/></td>
		    				<td><span id="amount" class="amount">0</span></td><td> <a href="javascript:void(0);" class="remCF">Remove</a></td>
								</tr>
   ...
   event.charCode == 46 || event.charCode == 0 "
						id="discount" class="form-control input-md double" name="discount"
						onKeyup="update_amounts();" value="${invoiceInfo.discount}" required /> <br />
				...
				<div class="input-group" id="inputDivRightBottom">
					<label class="lable" >totalCost * </label> <input type="number"
						...
						onkeypress="return (event.charCode >= 48 && event.charCode <= 57) ||  
   event.charCode == 46 || event.charCode == 0 "
						id="total" class="form-control input-md" value="${invoiceInfo.totalCost}" readOnly
						required /> <br /> <br />
				</div>
				...
				<div class="input-group" id="inputDiv">
					...
					<textarea rows="4" cols="50" id="input"
						class="form-control input-md" name="notes" value="${invoiceInfo.notes}" required></textarea>
					<br />
